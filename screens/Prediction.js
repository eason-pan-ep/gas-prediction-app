...
export default function Prediction({ navigation }) {
  ...
  useEffect(() => {
    ...
    const getPrediction = async () => {
      try{
        const q = query(collection(database, "predictionData"),where("user", "==", auth.currentUser.uid), where("date", "==", todayStr), where("location", "==", city));
        const querySnapshot = await getDocs(q);
        if(querySnapshot.empty){ //no match found => get new prediction data from the APIs
  ...
  const onPressClearCache = () => {
    ...
    Alert.alert("Clear Cache", "Are you sure you want to clear all prediction cache?", [
    ...
    <View>
      {predictions.length === 0? 
        null:<>
              ...
              <PredictionItem date={fiveDays[0]} price={`$ ${predictions[0]} / L`} />
              <PredictionItem date={fiveDays[1]} price={`$ ${predictions[1]} / L`} />
              <PredictionItem date={fiveDays[2]} price={`$ ${predictions[2]} / L`} />
              <PredictionItem date={fiveDays[3]} price={`$ ${predictions[3]} / L`} />
              <PredictionItem date={fiveDays[4]} price={`$ ${predictions[4]} / L`} />
            </>
      }
